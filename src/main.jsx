...
import { RotateCcw, Sparkles, BookOpen, Skull } from 'lucide-react';
    ...
    return (
      <main className="app">
        <section className="hero">
          <div className="badge">611研究室 原型版</div>
          <h1>因為不想改論文<br />我把技能全加在退件防禦了</h1>
          <p>事件相剋型生存卡牌遊戲。選擇角色，用三個絕招撐過隨機事件，活著抵達華衫論壇。</p>
        </section>
        <h2>選擇角色</h2>
        <div className="characterGrid">
          {characters.map((c) => (
            <button className="card characterCard" key={c.id} onClick={() => startWith(c)}>
              <div className="cardTop">611研究室</div>
              <h3>{c.name}</h3>
              <h4>{c.title}</h4>
